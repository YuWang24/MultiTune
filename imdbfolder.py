...
def prepare_data_loaders(dataset_names, data_dir, imdb_dir, shuffle_train=True, index=None):
    ...
    imdb_names_train = [imdb_dir + '/' + list(dataset_names)[i] + '_train.json' for i in range(len(dataset_names))]
    imdb_names_val   = [imdb_dir + '/' + list(dataset_names)[i] + '_val.json' for i in range(len(dataset_names))]
...
#                labels_train += labels
        ...
        means = dict_mean_std[(list(dataset_names)[i] + 'mean').encode('utf-8')]
        stds = dict_mean_std[(list(dataset_names)[i] + 'std').encode('utf-8')]

        if list(dataset_names)[i] in ['gtsrb', 'omniglot','svhn']: 
        ...
        elif list(dataset_names)[i] in ['daimlerpedcls']:
            transform_train = transforms.Compose([
            transforms.Resize(72),            
            transforms.ToTensor(),
            transforms.Normalize(means, stds),
...
#            transforms.RandomCrop(64),
        ...
        trainloader = torch.utils.data.DataLoader(ImageFolder(data_dir, transform_train, None, index, labels_train, imgnames_train), batch_size=120, shuffle=shuffle_train, num_workers=0, pin_memory=True)
        valloader = torch.utils.data.DataLoader(ImageFolder(data_dir, transform_test, None, None, labels_val, imgnames_val), batch_size=120, shuffle=False, num_workers=0, pin_memory=True)
